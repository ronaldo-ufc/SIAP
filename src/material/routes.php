...
use siap\material\models\Balanco;
...
$app->post('/produto/novo', function($request, $response, $args) {
    ...
    if ($msg[2]) {
        return $this->renderer->render($response, 'produto_novo.html', array('unidades' => Unidade::getAll(), 'grupos' => Grupo::getAll(), 'mensagemErro' => $msg[2]));
    }
...
$app->post('/confirmar-recebimento', function($request, $response, $args) {
    ...
    if (!$usuario) {
        $this->flash->addMessage('danger', 'Usuario não encontrado');
