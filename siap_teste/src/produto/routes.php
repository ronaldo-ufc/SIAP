...
$app->map(['GET', 'POST'], '/pdf', function($request, $response, $args) {
    ...
    $dompdf = new DOMPDF();
    $html = file_get_contents('gerar_pdf.html');
    // (Optional) Setup the paper size and orientation
    $dompdf->setPaper('A4', 'landscape');
    $header = '<div class="panel panel-default">
  <div class="panel-body"><h4 style="text-align: center;">UNIVERSIDADE FEDERAL DO CEARÁ<br />CAMPUS DE CRATEÚS<br />SISTEMA DE ALMOXARIFADO E PATRIMÔNIO - SIAP<br /></h4>
            <h4 style="text-align: center;">EMITIDO EM '.date("d-m-Y").' '.date('H:i:s').'</h4>  
</div>
</div>';
    

    return $this->renderer->render($response, 'gerar_pdf.html', array('dompdf'=>$dompdf,'array' => array("Attachment" => FALSE), 'header' => $header, 'html' => $html));
})->setName('Pdf');